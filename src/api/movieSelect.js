...
export const useMovies = () => {
  ...
  const selectMovie = async (id, keyword) => {
    ...
    if (isSelected) {
      ...
      setDisplayMovies(
        (prev) =>
          prev.filter(
            (movie) =>
              !cachedMovies.current[id]?.some((m) => m.movie_id === movie.movie_id) ||
              selectedMovies.includes(movie.movie_id)
          ) // 선택 하고 있는 영화는 화면에서 유지
      );
      return;
    }
    ...
    setDisplayMovies((prev) => {
      ...
      return [...prev.slice(0, insertIndex), ...filteredNewMovies, ...prev.slice(insertIndex)];
    });
  };

  const submitSelectedMovies = async () => {
    try {
      const response = await api.get('/moviechoice/keyword', {
        params: {
          keyword: selectedMovies,
        },
      });
